...
exports.run = async (client, message, args) => {
  ...
  this.fighting = new Set();
	...
	if (!opponent) return message.reply("<a:unlem:909343654081085481> Oynamak istediğin kişiyi etiketlemelisin!")
  
  if (opponent.bot) return message.reply('<a:unlem:909343654081085481> Botlar ile oynayamazsın!');
  if (opponent.id === message.author.id) return message.reply('<a:unlem:909343654081085481> Kendin ile düello atamazsın!');
		if (this.fighting.has(message.channel.id)) return message.reply('<a:unlem:909343654081085481> Kanal başına sadece bir düello meydana gelebilir.');
